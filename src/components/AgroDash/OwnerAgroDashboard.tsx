...
import { getOwnerFieldOfficersAgroStats } from "../../api";
...
const OwnerAgroDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const today = new Date(Date.now() - tzOffsetMs)
        .toISOString()
        .slice(0, 10);
      const cacheKey = `ownerAgroStats_${today}`;
      ...
      try {
        ...
        if (cached) {
          ...
          data = (await getOwnerFieldOfficersAgroStats(
            today,
...
export default OwnerAgroDashboard;
